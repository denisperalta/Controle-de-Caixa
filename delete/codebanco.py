...
class bancoApp(QtWidgets.QMainWindow, frmbanco.Ui_frmbanco):
    ...
    def guardar(self):
        ...
        if len(nom) > 3:
            ...
            if self.bnd == 0:
                cur.execute(
                    'insert into banco (nombanco) values(%s)', [nom])
            else:
                cur.execute('update banco set nombanco=%s where codbanco=%s', [nom, cod])
            con.commit()
            self.cancelar()
            self.consultar()
        else:
    ...
    def consultar(self):
        ...
        if int(rows[0]) > 0:
            ...
            self.config(rows)
            cur.execute('select codbanco, nombanco from banco order by codbanco asc')
    ...
    def eliminar(self):
        ...
        con = self.conexion.conectar()
        cur = con.cursor()
        cur.execute('select count(*) from conta where codbanco=%s', [cod])
        rows = cur.fetchone()
        if int(rows[0]) == 0:
            if QtWidgets.QMessageBox.question(self, "Eliminando Registro",
                ...
                con = self.conexion.conectar()
                cur = con.cursor()
                cur.execute('delete from banco where codbanco = %s', [cod])
                con.commit()
                self.cancelar()
                self.consultar()
